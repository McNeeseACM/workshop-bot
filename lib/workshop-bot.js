...
var sendChannelContextResponse = require('./channel-context-response');
  ...
  if (!this._isFromWorkshopBot(message)) {
    ...
    if (this._isChatMessage(message)) {
      ...
      if (this._isChannelConversation(message)) {
        ...
        sendChannelContextResponse(this, message);
    ...
    } else if (this._isUserJoiningMessage(message)) {

      userName = this._getUserNameById(message.user.id);
      this._sendGreetingToNewUser(userName);
 ...
 * @param  {Object} channelId [description]
...
/**
 * Determines if message is notification of user joining
 * 
 * @param  {Object}  message [description]
 * @return {Boolean}         [description]
 */
WorkshopBot.prototype._isUserJoiningMessage = function(message) {
  return message.type === 'team_join' && !!message.user;
}

/**
 * Get User name given string ID
 * 
 * @param  {String} userId [description]
 * @return {String}        [description]
 */
WorkshopBot.prototype._getUserNameById = function(userId) {
  var userList = this.getUsers()._value.members;

  var user = _.find(userList, function(usr){
...
/**
 * Sends greeting to new user given user name
 * 
 * @param  {String} userName [description]
 */
WorkshopBot.prototype._sendGreetingToNewUser = function(userName) {
  this.postMessageToUser(
    userName,
    this.settings.new_user_greeting,
    {as_user: true}
  );
};
